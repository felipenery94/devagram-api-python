...
MONGOBD_URL = config("MONGOBD_URL")


cliente = motor.motor_asyncio.AsyncIOMotorClient(MONGOBD_URL)
...
async def criar_usuario(usuario :ModelCriarUsuario) -> dict:

    usuario.senha = gerar_senha_criptografada(usuario.senha)

    usuario_criado = await usuario_collection.insert_one(usuario.__dict__)

    novo_usuario = await usuario_collection.find_one({"_id": usuario_criado.inserted.id})
...
async def deletar_usuario(id: str):
    usuario = await usuario_collection.find_one({"_id": ObjectId(id)})
    if usuario:
        await usuario_collection.delete_one({"id":ObjectId(id)})
